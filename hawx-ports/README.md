...
and so it must provide an interface to allow user processes to use these
devices.
...
Then, expand the file by running the following command:
    ...
    tar xzf hawx-ports.tar.gz

This will create the directory "hawx-ports". Change into this
...
As HAWX is based in part on xv6, you can also run the debugger as you
could in xv6. As it is now, when you run HAWX, you will see the
following output
...
Finally, at the bottom of the `port.c` file, you will see the
`port_test` function. This is the unit test that checks that your
functions work.  You are not allowed to alter this function, but feel
free to read it as it may give you clues to the expected behavior of
the port system.
...
This will create the file `hawx-ports-turnin.tar.gz` in the directory
above hawx-ports. Upload this tar.gz file to canvas to submit your
assignment.
